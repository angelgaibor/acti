...
marco_sin_inc_for <- marco |> 
  filter(tam_acti != "G4") |> 
  mutate(estrato = paste0(codigo_provincia, codigo_division, tam_acti))

muestra_inc_for <- marco |> 
  filter(tam_acti == "G4")

resumen_muestra_inc_for <- muestra_inc_for |> 
  group_by(codigo_division, tam_acti, codigo_provincia, gsectores) |> 
  summarise(Nh = n(),
            nh = n())

#------------------------------------------------------------------------------#
#                          Parametros tamaño muestral ficha 1
#------------------------------------------------------------------------------#
...
# se trabaja con los dominios tamanio, codigo_division, provincia
...
distribucion <- marco_sin_inc_for %>% 
  ...
  mutate(nh_emp_ta = case_when(is.na(nh_emp_ta) ~ pmin(Nh, 2),
         ...
         nh_ven_ta = case_when(is.na(nh_ven_ta) ~ pmin(Nh, 2),
                               ...
                               T ~ pmin(Nh, nh_ven_ta))) |> 
  left_join(tam2 %>% 
              select(codigo_provincia, n_emp_cp = tam_empleo, n_ven_cp = tam_ventas), 
  ...
  mutate(nh_emp_cp = case_when(is.na(nh_emp_cp) ~ pmin(Nh, 2),
         ...
         nh_ven_cp = case_when(is.na(nh_ven_cp) ~ pmin(Nh, 2),
                               ...
                               T ~ pmin(Nh, nh_ven_cp))) |> 
  left_join(tam3 %>% 
              select(codigo_division, n_emp_cs = tam_empleo, n_ven_cs = tam_ventas), 
            by = "codigo_division") %>% 
  group_by(codigo_division) %>% 
...
ajuste <- distribucion |> 
  select(codigo_division, tam_acti, codigo_provincia, gsectores, Nh, nh) |> 
  mutate(dominio = paste0(substr(tam_acti, 2, 2), substr(codigo_division, 1, 1))) |> 
...
resumen_muestra_final <- resumen_muestra_inc_for |> 
  rbind(ajuste |> select(codigo_division, tam_acti, codigo_provincia, gsectores, Nh, nh = nhf))

sum(resumen_muestra_final$nh)

export(resumen_muestra_final |> 
         mutate(codigo_provincia = str_pad(codigo_provincia, 2, "left", "0")), 
       "pedidos/002/ficha1/tamaño_ficha1.xlsx")
